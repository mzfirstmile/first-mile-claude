...
## Supabase Project
- **Mobile login fix (2026-09-09):** `no_state_in_hash` on phones was caused by the hash router IIFE at the bottom of index.html — it treated the MSAL response hash `#code=…&state=…` as a view name and `history.replaceState('#code=…')` stripped `state` before MSAL read it. Fix: `isAuthHash()` guard in `getViewFromHash`, the initial-load IIFE and `popstate`; mobile UAs / <768px now use `loginRedirect`/`logoutRedirect` + `handleRedirectPromise()` in `msalInit` (desktop keeps popups). **Git note:** the Dropbox checkout's `.git` is stale (behind origin) and the mount forbids unlink, so `git commit` there fails with a stuck `index.lock`. Commit/push from a fresh clone in the local VM (`git clone` in `$HOME` works from device_bash; the cloud container's proxy blocks this repo). `scripts/_to_delete_tmp/` in Dropbox is a leftover patch file — safe to delete.
...
- **Shortlist (Phase 1):** 1,865 towns that pass the demographic pre-screen (pop 4k–75k AND median HHI ≥ $100k). Tagged `phase='shortlisted'` in `market_research_markets`. **Threshold lowered from $130k → $100k on 2026-05-19** to expand the Tier 3 pool. As of that change, 819 of the 1,865 still need Phase 2 scoring (newly promoted from universe — Phase 2 has to be re-run from Chrome MCP to score them).
- **Pipeline status 2026-09-01:** 1,873 shortlisted. Composite recompute had never run for 116 towns that had full Phase 2+3 score rows (Lexington MA, Needham MA, Menlo Park CA, Merion Station PA, Larchmont NY…) — they sat with NULL score/tier and were invisible in rankings. Fixed by running the `_updateRankings` composite SQL server-side + re-ranking. Also tier CASE now uses the ROUNDED score (was raw, so 8.45→"8.5" displayed as T2). Remaining research backlog: **Phase 3 not yet run on 208 residential-T2 towns** (NY 22, MD 21, NJ 16, OH 16, FL 13, CA 13, IL 12…), 484 T3, 80 T4; office-view Phase 3 missing on 309 office-T2 / 361 office-T3; 196 towns with Phase 3 done carry only residential values (no `value_numeric_office`) — those show "—" in office-view category cells and Area Reports. 3 shortlisted towns have zero score rows.
  - **Phase 3 run of 2026-09-01 (459 towns, sandbox subagents — API key was blocked by an Anthropic billing sync issue):** all office-T1/T2 towns lacking Phase 3 were scored via Claude Sonnet subagents inside the Cowork session using the edge function's exact system prompt, then ingested with `/home/claude/p3/ingest.py` logic (delete+insert `phase3_claude` rows incl. `value_numeric_office`, PATCH thesis/summary/phase3_ran_at, full composite recompute + re-rank). **Method differences vs. the May run:** (1) knowledge-based values carry an `(est.)` suffix and `sources` are verification pointers, not claimed-consulted citations; (2) for the first ~200 towns the Niche.com page was actually fetched (WebSearch+WebFetch) to verify Niche grade + violent/property crime per 100k (national baselines 380 / 1,950 per 100k) — the session's 200-search budget then ran out and the remaining ~260 towns are knowledge-only; (3) `Commute to Major Metro` and `Proximity to Major Highway` were NOT overwritten (kept programmatic phase2 values); (4) nulls used freely for obscure CDPs. **Calibration on 5 previously-scored elite towns: new method scores ~1.0 pt lower on average (MAD 1.26)** — the May run was more generous. Consequence: today's 459 towns landed 150 office-T2 / 309 office-T3 (21 res-T2 / 438 res-T3). For like-for-like comparability, re-run the May 1,100 with this method someday (or accept that May scores skew high). Skipped (too few scorable criteria): Waller WA, Buckhead GA, Skyline View PA, Potosi TX. **Also:** the full composite recompute revealed stored residential composites for the May towns were stale vs. current weights — residential T1 dropped 226→141 on recompute (office T1 unchanged at 383). That's what the Update Rankings button would have done; not a data loss.
  - **Pipeline status 2026-09-09:** Phase 2 is COMPLETE on all 1,873 shortlisted towns (the "339 unscored" were only missing the `phase2_ran_at` timestamp from the May browser run — backfilled; 3 truly unscored FL towns — Gotha, Lake Butler, Winter Park — were scored via the edge function). Office T1/T2 Phase 3 is complete except the 4 deliberately-skipped CDPs (Potosi TX, Waller WA, Buckhead GA, Skyline View PA). Remaining Phase 3 backlog: 219 office-T3 + 91 office-T4 (all residential T3/T4). **Office data-gap fix:** the 1,673 `phase2_commute` rows (`Commute to Major Metro`, active in office view only) had `value_numeric` but NULL `value_numeric_office`, so the office composite had silently ignored commute time since May. Copied `value_numeric → value_numeric_office` for phase2 rows where `target_min_office IS NOT DISTINCT FROM target_min`, recomputed composites (canonical SQL with `is_active_*` filters) + ranks. Effect: 503 towns' office scores moved (mean +0.29, range −1.7…+1.9), 133 towns moved up an office tier / 3 down. **Office tiers now 448 T1 / 939 T2 / 448 T3 / 38 T4** (was 383/850/549/91). Residential unchanged (142/860/786/85). Pre-fix office snapshot saved in session only — not restorable from DB. **Gotcha:** the Phase 2 edge function writes only `value_numeric` (no `value_numeric_office`, no `raw_value`, no office composite) — after any Phase 2 run, copy the office value and run the composite recompute. **Also fixed:** `_finishCriteriaEdit`'s composite SQL (market-research.js ~L3419) lacked the `is_active_residential/office` filters and used unrounded tier cutoffs — running it inflates residential T1 to ~204. Now identical to `_scheduleRecomputeAll`'s canonical version.
  - **Office-view Phase 3 values are a straight copy of residential** (`value_numeric_office = value_numeric` for `updated_by='phase3_claude'`; the edge function only writes `value_numeric`). 2026-09-01: back-filled the 4,429 phase3 rows that were missing the office copy, recomputed composites + ranks. Any future Phase 3 run must copy the value into `value_numeric_office` too (or run that UPDATE after). Phase 3 economics from the edge function: ~$0.04/town actual (sonnet-4-6, 3k max_tokens), batches of 20 at concurrency 8 ≈ 60–100s per call.
- **Tier bands** (on composite Phase 2 score, 0-10): **T1 ≥ 8.5 · T2 7.0-8.4 · T3 4.0-6.9 · T4 < 4.0**. Bands updated 2026-05-19 (was 8.5/6.5/4.5) to widen T3 — old bands left only 14 towns in T3 because the affluent shortlist self-selects high scores. New distribution on the 1,046 scored towns: 246 T1 / 678 T2 / 117 T3 / 5 T4. Logic lives in `market-research.js` `_recomputeMarketScore` (line ~2009).
- **Tables (Supabase):**
...
- **Pipeline phases (all DONE end-to-end as of 2026-05-18):**
  ...
  2. **Phase 2 — Programmatic scoring on the shortlist (DONE).** Re-queries Census ACS per state for B19001 (HHI brackets) + B15003 (education attainment) on top of Phase 1's B01003/B19013/B25077. Parses the Big Four / Wealth Mgmt 60mi Excel from Dropbox and groups offices by Nearest-Top-50-City. For each shortlist town, scores ~13 sub-criteria 0-10 across **Demographics**, **Education**, and **Company Concentrations** (the 3 categories Census + Excel data can reach without web research). Rolls up to category-weighted composite Score (1-10) + Tier (1=8+, 2=6-8, 3=4-6, 4=<4). Writes 8,399 score rows + PATCHes every shortlist market with score/tier/phase2_ran_at. **Implementation (2026-05-19):** Lives in Supabase edge function `market-research-phase2` (`supabase/functions/market-research-phase2/index.ts`). Sandbox invokes via `curl $SUPABASE_URL/functions/v1/market-research-phase2`. Edge function reaches api.census.gov server-side (no CORS issues). Original 2026-05-18 run was browser-driven via Chrome MCP from a CSP-permissive tab, but Census now blocks cross-origin fetches from example.com — DO NOT try Chrome MCP for Census, use the edge function. Free.
     - **Scoring formulas (reverse-engineered from existing scores, confirmed exact match):**
       - **Town Population** (range 5k-75k, peak 25k-50k): tent function. `<5000`→0, `5000-25000`→`(pop-5000)/2000` (linear ramp), `25000-50000`→10, `50000-75000`→`(75000-pop)/2500` (linear decay), `>75000`→0. Round to 1 decimal. Validated: pop 7140 → 1.07, pop 51508 → 9.38, pop 72070 → 1.19.
       - **Median Household Income** (target $200k, top-coded $250,001): `min(10, mhi/20000)`. Validated: $120k→6.0, $250k→10. value_text = `$N` formatted.
       - **Median Single-Family Home Price** (target $1.5M): `min(10, hv/150000)`. Validated: $251,700→1.7, $2M+→10.
       - **Households Earning $200k+** (target 35%): `min(10, pct*10/35)` where pct = 100 × B19001_017E / B19001_001E. Validated: 9.7%→2.8, 11.8%→3.4. value_text = `N%` formatted to 1 decimal.
       - **Bachelor's Degree Attainment** (target 70%): `min(10, pct*10/70)` where pct = 100 × (B15003_022+023+024+025) / B15003_001E (i.e., bachelors + masters + professional + doctorate of 25+ pop). Validated: 14.7%→2.1, 22.5%→3.2.
       - **Professional / Graduate Degrees** (target 30%): `min(10, pct*10/30)` where pct = 100 × (B15003_023+024+025) / B15003_001E. Validated: 3.7%→1.2, 4.5%→1.5, 5.0%→1.7.
       - **General formula** for any `target_min`-only criterion: `score = round(min(10, value/target_min*10), 1)`.
     - **Census ACS variables used:** `B01003_001E` (population, already in Phase 1), `B19013_001E` (MHI), `B25077_001E` (median home value), `B19001_001E + B19001_017E` (total HHs, HHs $200k+), `B15003_001E + B15003_022E + 023E + 024E + 025E` (25+ pop, bachelors, masters, professional, doctorate). Single ACS call per state covers all of these for all places. API endpoint: `https://api.census.gov/data/2022/acs/acs5?get=NAME,B19001_001E,B19001_017E,B15003_001E,B15003_022E,B15003_023E,B15003_024E,B15003_025E&for=place:*&in=state:{FIPS}` (no key needed for ≤500 calls/day).
     - **Company Concentrations (6 criteria — Big Four / Wealth Mgmt / Total Tenant-Base / Tenant Sector Diversity / 2 flagship counts):** Originally parsed from `Big_Four_and_Wealth_Mgmt_Within_60mi.xlsx` in Dropbox, grouped per `nearest_top50_city`. For incremental Phase 2 runs (e.g. when new towns are promoted to shortlist), inherit the scores from same-metro peers — `SELECT s.value_numeric, s.value_text FROM market_research_scores s JOIN market_research_markets m ON s.market_id = m.id WHERE m.nearest_top50_city = $1 AND s.updated_by = 'phase2_auto' AND c.category = 'Company Concentrations' LIMIT 1 per criterion`. Tenant Sector Diversity is hard-coded to 10 across all rows.
     - **Composite + tier** (final step in scoring loop): for each scored market, group its score rows by category, average within category, then weighted-average across categories using `market_research_categories.weight`. Round to 1 decimal. Tier bands (2026-05-19): T1 ≥ 8.5 / T2 7.0-8.4 / T3 4.0-6.9 / T4 < 4.0. See `_recomputeMarketScore` in market-research.js line ~2009 for the canonical JS version.
     - **Deployment + invocation:** `supabase functions deploy market-research-phase2`. Trigger from sandbox: `curl -X POST $SUPABASE_URL/functions/v1/market-research-phase2 -H "Authorization: Bearer $SUPABASE_ANON_KEY" -H "Content-Type: application/json" -d '{"batch_size":200,"only_unscored":true}'`. Batches of 200 keep each call under the 150s Pro-plan edge-function timeout. Returns `{processed, score_rows_written, remaining, done}` so the caller can loop until `done:true`.
...
- **UI (market-research.js):**
  ...
  - **Market Area Report (2026-09-01):** when an address pin is active, a `📄 Area Report (PDF)` button appears in the address hint bar (`#mrAreaReportBtn`, fn `_buildAreaReport` / `window.mrAreaReport`). Takes the **nearest 10 markets with a score in the active view** (residential or office) from the cached `_proximityAll` list, fetches their score rows (one call per market to dodge the 1000-row cap), and opens a print-ready HTML report in a new tab (letter landscape, "Save as PDF" button → `window.print()`). Sections: KPI strip (area score, simple avg, best market, tier counts, avg HHI/pop) · **Triangulation SVG** (subject at center, rings auto-scaled to farthest market, markets plotted by bearing/distance, dot color = tier, dot size ∝ score) · category radar (area-weighted polygon vs top-3 markets) · category × market heat-map matrix with an **Area** column · market roster · appendix of criterion-level scores. **Area score = distance-weighted average, weight = 1/(miles+2)** so nearest towns dominate; area tier uses the standard bands. No external libs — all inline SVG/CSS so it prints anywhere. Logo pulled from admin.firstmilecap.com/assets.
...
- **67 East 161st — Bronx Defenders Ground-Up Development initiative (added 2026-05-12):** id `36bcaad5-dd15-4daa-a542-102a290e62da`. Category `prospective_investment`. Ground-up office/retail development at 67 East 161st St, Bronx NY (Bronx Hub, across from Yankee Stadium / opposite Bronx County Courthouse). **Two-parcel land assemblage:** McDonald's parcel $9.25M (PSA signed 4/30/2026, subject to 120-day Warren Act notice to franchisee, ~24-mo build window) + Chase parcel $8.50M (PSA signed June 2026, closing Dec 2026, Chase relocates branch to LX site across the street) = **$17.75M land** on 20,510 SF lot at 7.65 FAR → **157,000 SF NLA** ($865/SF land basis, $113/SF buildable). **Build-to-suit anchored by Bronx Defenders:** Suite 300 = 150K SF @ $55/SF NNN, 15-yr term, RCD 12/31/2029; Suite 101 = 7K SF supermarket-grade retail @ $50/SF NNN, 10-yr (placeholder, supermarket LOI in folder). Blended $54.78/SF. **UW v5 (5.4.26) base case:** Total Project Cost $104.0M ($663 all-in basis/SF). Hard $56.9M ($381/SF MJM) + Soft $7.65M ($44/SF) + Contingency $3.2M + TI $1.89M + LC $4.61M + Dev Fee $2.71M (4%) + Cap Int $7.61M + Loan Fees $0.31M + Land/Closing $19.15M. Construction loan $67.6M @ 7.50% (65% LTC, 36-mo IO) → refi at Month 36 to permanent $98M @ 6.10% (3.60% fixed + 250 bps, 30-yr am, sized by 1.20x DSCR). Stabilized YoC 8.24%, Debt Yield 6.32%. 72-mo hold, 5.50% exit cap → Sale $155.4M ($990/SF) → **Levered IRR 19.84% / 2.03x / $41.7M profit** on ~$45.9M equity (unlevered 12.06% / 1.69x). **Sensitivity** (Levered IRR/EM, 72-mo hold): 4.50% cap 28.1%/2.84x, 5.00% 23.9%/2.40x, **5.50% 19.8%/2.03x (Base)**, 6.00% 15.7%/1.72x, 6.50% 11.4%/1.46x. **Members:** mz (owner), ek (Ehud Kupperman, member), aiassistant (member). NOT shared with rc/ty per Morris's selection on creation. **Construction:** MJM is GC; latest hard-cost budget dated 4/10/2026, prior soft-cost budget 2/13/2026. **Plans:** KSS Architecture (filename prefix 21-126); latest plan set 5/11, latest zoning presentation 3/23/2026. **Tenant negotiation status:** BXD redline response received 11/21/2025 — open economic terms in lease still in play; work letter draft on file. **Open data needs:** Bronx Defenders LOI to executed lease, Chase PSA closing confirmation, construction loan term sheet, supermarket tenant LOI execution. **Dropbox source folder:** `1.2 Projects - Diligence/161st St Chase/Bronx Defenders/` (Underwriting/, Tenant LOI/, Chase LOI/, Construction - MJM/, Plans Zoning Presentations/, Test Fit/, Tenant Work Letter/, Pre-closing Invoices/, Cost Tracking xlsx). **Methodology note:** Stabilized DSCR in model reads 0.78x — likely artifact of TRO drawdown timing (ANOI net of $8.05M tenant remaining obligation); flag for lender walkthrough before financing application.
...
## AI-Ready Spec Suite — 3D Test Fit (added 2026-07-03)
- **What:** Interactive three.js walkthrough of a prototype "AI-ready" spec suite (140′×72′ = 10,080 RSF corner suite, glass curtain wall on south + east; expanded from 5K per Morris 2026-07-03). Driven by Morris's AI-office thesis email (HPC retrofits, zero-trust silos, elastic leases). File: `spec-suite-3d.html` (single file, three.js r128 from cdnjs, no build step).
- **Initiative:** "AI-Ready Spec Suite — Test Fit Program" id `c91a29d0-8203-4a30-994b-476364579242`, category `general`. Members: mz (owner), rc, ty, aiassistant. Has pinned strategy note, 5 milestones, key_metrics deal table (component cost/vendor matrix), and a document entry with `metadata.button_label` + `metadata.url`.
- **Link-button mechanism (initiatives.js):** detail header renders a gradient CTA button for ANY entry with `metadata.button_label` + `metadata.url` (fn `_updateLinkButtons`, container `#initLinkBtns`). This initiative's entry shows "🧊 View 3D Walkthru" → `spec-suite-3d.html` (relative URL works on localhost + prod).
- **16 annotated components**, each with est. cost range, basis, "Why the AI office needs it" rationale, hyperlinked product vendors (open in new tab), and installer shortlist: 5 acoustic pods, Immersive Matrix Room, spatial audio, kinetic walls, smart glass (toggle demo), liquid-cooled edge-AI closet, zero-trust closet, power upgrade 12–15 W/SF, high-STC shell, wet pantry, 4 glass-front exec offices, reception/entry (glass storefront double doors), M/W ADA restrooms, connectivity backbone (fiber + private 5G), sensor mesh & building OS, resilient power (UPS). Totals: ~$1.6M–$3.1M ($160–$310/SF on 10,080 RSF).
- **UI:** Auto Tour, Walk View (WASD + drag), Plan View (centered aerial), smart-glass + kinetic-wall animations, in-scene floating toggle buttons anchored on the smart glass + kinetic walls, 📐 Dimensions toggle (off by default; CAD-style linear dimension lines w/ extension ticks at ceiling height + per-room area tags), label occlusion via raycast (tags hide behind opaque walls, show through glass). Costs are rough 2026 NY/NJ planning estimates — validate with vendor RFPs.
- **Layout (v4 final, 17 components, $2.0M–$3.8M / $200–$380/SF):** corner suite, glass on S+E + full-glass storefront west wall w/ double glass entry doors + timber canopy. Reception (desk faces entry, waiting lounge clear of door path), 8 acoustic pods on north wall (4 west + 4 east between RRs and IT; Acoustic Shell tag sits ON a pod per Morris), wet pantry NW w/ L-shaped counter run, glass-walled tenant lounge adjacent to pantry, two 12′×10′ restrooms (2 stalls + double vanity each), server+network rooms at NE interior corner (glass fronts), 5 inline exec offices 12′×13′ + SE corner exec office (execs back-to-window facing door), 2 AI huddle rooms (10′×10′), **2 Immersive Matrix Rooms side-by-side on the east window line (east wall = exterior curtain wall)**, collab hall (single band, 2 kinetic runs), flex desking ringed by kinetic walls (converts to conf), desking cluster beside Matrix rooms. People staged (mannequins), greenery/decor (green wall on RR front, pillows, books, console in lounge). In-scene toggle buttons (smart glass / kinetic walls) show only at eye level within 40 ft; CAD dimension lines at ceiling height via 📐. **Deal table in Supabase synced to 17-component version 2026-07-03 (entry 7a109e3a).**
- **Rendering realism:** ACES tone mapping + sRGB, PMREM env map, procedural canvas textures (wood/carpet/plaster/fabric), contact-shadow blobs under furniture, parametric furniture helpers (chairT/sofa/tableLegs/plantPot/stool/person), 7 staged human figures, wood-floor zones at reception/pantry, wall art, baseboards. Per Morris: NO tower shell / city skyline (too cluttered) — office only on a slab against a sky gradient; intro camera sweeps in over the SW corner with reception in foreground.
- **Known gotchas:** `renderer.setSize(w,h,false)` requires CSS `width:100%;height:100%` on the canvas or it displays at buffer size (bug found + fixed). rAF is throttled in occluded tabs — camera fly-to appears frozen in automation contexts, fine for real users.
- **Pending:** git push to deploy to admin.firstmilecap.com (sandbox couldn't reach GitHub this session; file sits in working tree). Supabase deal-table update to the 16-component list — Morris said park it ("forget supabase lets finish the project first"). Deal table currently shows the earlier 13-row version… actually 14-row update also parked; sync when asked.
...
- **Property Financials — Status-aware tab bar (2026-04-22, reworked 2026-05-11):** Tabs in the Property Financials detail panel change based on the selected year's status:
  - **Active year** (e.g. 2026): `Overview | P&L | Rent Roll | Stacking Plan | Cash Forecast | Balance Sheet`. Lands on Overview by default.
  - **Planning year** (e.g. 2027): `P&L | Rent Roll | Stacking Plan | Cash Forecast | Balance Sheet`. Lands on P&L.
  - **Historical year** (e.g. 2025): `P&L | Variance Story | Rent Roll | Stacking Plan | Balance Sheet`. Lands on Variance Story.
  - Tab config lives in `FB_TABS_BY_STATUS` + `FB_DEFAULT_TAB`. Rebuilt dynamically by `fbUpdateNavForYear()`. Legacy tab ids (`pl`, `summary`, `revenue`, `opex`, `capital`, `monthly`, `trend`, `drill`, `budget`, `actuals`) alias forward to the unified `pnl` tab via `FB_LEGACY_TAB_MAP` so old URL hash deep-links keep working.
- **Unified P&L Tab (2026-05-11):** Collapsed the old `P&L / Monthly Trend / Drilldown` trio (active year), the `Budget / Drilldown` pair (planning), and the lone `Actuals` tab (historical) into a single **P&L** tab with view controls and a slide-in drilldown drawer. Renderer: `fbRenderPLTab(body, p, baseProp)` in index.html near line 7198.
  - **Period toggle:** Annual / Quarterly / Monthly. Annual delegates to existing `fbRenderSummary`; Quarterly/Monthly delegate to `fbRenderMonthlyActVsBgt` (which keys off `fbMonthlyGrouping` — kept in sync with `fbPLPeriod` in both directions via `fbSetPLPeriod` and `fbSetMonthlyGrouping`).
  - **Compare chip:** Read-only label for v1 (`{yr} Blended vs Budget vs {yr-1}` on active, `{yr} Budget vs {yr-1} Actuals` on planning, `{yr} Actuals vs {yr} Budget vs {yr-1}` on historical). Column logic stays inside each renderer's status-aware code path. Hook up a real override later if needed.
  - **Row-click drilldown drawer:** Clicking any account row in the P&L table opens `#fbDrillDrawer` — slides in from the right (`min(720px, 78vw)`), backed by `#fbDrillOverlay`. Drawer wraps `fbRenderMonthlyDetail` with a Revenue / OpEx / Capital & Debt segmented switcher at the top. Clicked label is shown as a "scroll to find <b>{label}</b>" hint. Section is inferred via `fbInferDrillSection(label)` — regex on income/rent/recover/cam keywords → 'revenue'; depreciation/debt/capital/TI/LC keywords → 'capital'; everything else → 'opex'. Auto-closes on tab switch, ESC key, or overlay click. State: `fbPLPeriod`, `fbDrawerOpen`, `fbDrillSection`.
  - **Mockup file:** `mockups/pnl-tab-redesign.html` — standalone HTML preview used to design this iteration.
...
- **Planning Year — Budget Cell Entry (2026-05-11):** In the P&L tab's Monthly view, when the selected year status is `planning`, every Reg-type GL row renders 12 editable `<input>` cells (one per month) for the Budget column. The Act and Var columns stay read-only. Quarterly view is intentionally NOT editable (multiple months combine into one cell → ambiguous for entry).
  - **What you see is what saves:** Inputs use `fbFmtFull` ($12,345) instead of the abbreviated `fbFmt` ($12.3K) so values are not lossy. Parser strips `$`, commas, whitespace.
  - **Save flow:** On blur or Enter, the handler does optimistic local-state update → `supaWrite('budget_line_items', 'DELETE', null, ?property_id&year&gl_code&month)` → `supaWrite('budget_line_items', 'POST', [...])`. If amount is 0, skips the POST (DELETE alone). On error, reverts local state and alerts.
  - **In-place totals:** After each save, `fbRecomputeBudgetTotsInPlace` walks every Tot-type row, recomputes its 12 monthly values from child Reg rows whose gl_code falls inside `_totRange = [lo, hi]`, and updates the visible cells in-place. No full re-render — focus + tab order survive across edits. Row-total cells (`.fb-bgt-row-total`) also refresh.
  - **Skeleton metadata:** The budget loader's `summaryRows.push(...)` now attaches `gl_code`, `gl_type` (Head/Tot/Reg), and `_totRange` (for Tot rows) to every row so the renderer can decide what's editable and the totals-recomputer can find children.
  - **Keyboard:** Tab moves across months (native), Enter moves down to the next editable cell in the same month (custom handler), Escape reverts the in-progress edit.
  - **Visual cues:** Editable cells get a soft cyan background tint, hover shows darker tint, focus puts a 2px accent border inset. Saving = small pulsing dot top-right; saved = green ✓ for 700ms; error = red background for 2s plus alert.
  - **Banner:** When budget-entry mode is on, a callout at the bottom of the table reminds: "Budget entry mode. Click any monthly budget cell to edit. Tab/Enter to confirm. Changes save automatically to `budget_line_items`."
  - **Access:** All authenticated dashboard users can edit (no role gate in v1). Lock down later via Supabase RLS if needed.
...
- **Straight Renewal profile (auto-seeded, 2026-05-11):** Every property automatically gets a "Straight Renewal" profile — the antenna / rooftop / storage / parking pattern where the tenant just keeps going at LXD at the same rent with no downtime, no free rent, no TI, no LC, and no escalation bump. Settings: `renewal_probability_pct=100`, `base_rent_psf=NULL` (so the projection carries forward each tenant's current rate — works for both PSF leases and flat-monthly antenna leases via the new `monthly_rent` fallback in `profileToProjection`), every new/renew lever = 0. Card shows a green "STRAIGHT RENEWAL" badge. Auto-creation: when a property's Rent Roll tab loads, if no profile named "Straight Renewal" exists for that property, one is POSTed silently in the background and added to the local cache. Backfill SQL for existing properties: `migration/seed-straight-renewal-profiles.sql` (idempotent — only inserts where missing, safe to re-run). `profileToProjection` was extended so that when a profile has NULL `base_rent_psf` AND the tenant has no SF (antenna leases), it falls back to `r.monthly_rent` directly — so $7,250/mo flat antenna rent keeps billing $7,250/mo after rollover instead of dropping to $0.
- **Per-tenant override values (2026-05-11):** The rent-roll table now exposes 11 inline-editable cells to the right of the Re-Leasing Profile dropdown, letting Morris override any of the profile inputs for a specific tenant: Base Rent ($/SF), Downtime (New/Renew, months), Free Rent (New/Renew, months), Escalation (New/Renew, %/yr), TI (New/Renew, $/SF), LC (New/Renew, %). Each cell shows the tenant's override if set; otherwise it shows the assigned profile's value as a placeholder. Empty input reverts to the profile. Two-row table header groups the 11 columns under a "Re-Leasing Override Values — blank cells inherit from profile" super-header. Migration: `migration/add-rent-roll-overrides.sql` adds 11 nullable columns (`rl_base_rent_psf`, `rl_new_downtime_months`, `rl_renew_downtime_months`, `rl_new_free_rent_months`, `rl_renew_free_rent_months`, `rl_new_escalation_pct`, `rl_renew_escalation_pct`, `rl_new_ti_psf`, `rl_renew_ti_psf`, `rl_new_lc_pct`, `rl_renew_lc_pct`) to `rent_roll`. Persist handler: `fbSetTenantOverride(rentRollId, col, rawValue)` — whitelisted column name, coerces empty → null, PATCHes `rent_roll`, patches local cache in place. `profileToProjection` was refactored to resolve each lever via `ov(rlKey, prKey)` → tenant override wins, profile is fallback. Renaming/clearing an override re-fetches the profile value at projection time.
- **Stacking Plan tab (2026-05-11):** New tab on Property Financials (between Rent Roll and Cash Forecast on Active/Planning, after Rent Roll on Historical). Floor-by-floor visualization of the rent roll. Each floor is one row; tenant blocks are sized proportionally to leased SF as a share of their floor's total. Floors are inferred from suite numbers — `fbInferFloor(suite)` returns the first digit of a numeric suite (e.g. 460A → F4), or maps ROOF* → Roof row, PARK/BOX/SHACK/SIGN/KIOSK → Ground row. Sort order: Roof at top, then F5→F1 descending, then Ground, then Other. Blocks colored by category (office / telecom / parking-license) and given a 2px red border when expiring < 12 months from today, 2px amber for 12–24 months. Hover tooltip shows full tenant name + SF + exact expiration date + monthly rent. Footnote calls out building SF that isn't accounted for in the rent roll. Renderer: `fbRenderStackingPlan(body, p, baseProp)` near line 5101. Reuses `fbRentRollCache` so the plan stays in sync with whatever the user sees on the Rent Roll tab.
- **Sync preserves user overlays (2026-05-11, CRITICAL fix):** `sync_rent_rolls.py` historically did DELETE+INSERT per property, which silently nuked every per-tenant `releasing_profile_id` and `rl_*` override value on each re-sync. (Found this after Morris reported losing all his 61 Paramus profile assignments.) `supabase_upsert()` now snapshots `USER_OVERRIDE_COLUMNS = (releasing_profile_id, rl_base_rent_psf, rl_new/renew_downtime_months, rl_new/renew_free_rent_months, rl_new/renew_escalation_pct, rl_new/renew_ti_psf, rl_new/renew_lc_pct)` keyed by `(tenant_name, suite)` BEFORE the DELETE, then PATCHes the overlay values back onto the matching new rows AFTER the INSERT. Includes a row-match verification using `Prefer: count=exact` + `Content-Range` so renamed tenants surface as a "skipped: tenant=… suite=… — likely renamed in Yardi" warning rather than silently dropping. Falls back gracefully to snapshotting just `releasing_profile_id` if the `rl_*` migration hasn't been run. **Restoration done 2026-05-11:** 32/32 61 Paramus tenants re-assigned via SQL Console (Straight Renewal for ROOF/BOX/PARKING, UBS/MS Renewal for those tenants, > 25K SF / > 5K SF / < 5K SF by SF band).
...
## Red Bank — River Center Property (added 2026-05-11)
- **What:** Added Red Bank / River Center as a managed property `recRBRiverCentr01` (Yardi code `p0000008`). 5-building Class A office campus in Red Bank NJ: 100 Schultz Dr, 200 Schultz Dr, and River Center 1/2/3 at 331 Newman Springs Dr. Total ~687,043 SF (sum of Paramount Realty's per-building PROPERTY TOTALS). Entity: FM Red Owner LLC. Owner JV: FM Paramount JV LLC (FM 70% / Paramount Realty 30%, see Red Bank initiative notes).
- **Source:** Paramount Realty monthly reporting package `_RB Report 4.26.pdf` (Apr 2026, period ending 4/30/2026) uploaded by Morris.
- **Rent roll imported:** 73 rows total — 38 occupied tenants + 35 vacant suites across the 5 buildings. Suite naming: `{building_code}-{unit}` (e.g. `rivrctr1-103`, `200schlz-101`, `100schlz-101`). Anchor tenants: NVA Red Bank Veterinary 108,500 SF @ 100schlz ($3.72M annual base), LTF Lease Co (Lifetime Fitness?) 40,212 SF @ rivrctr3 ($985K), Peraton Labs 27,635 SF @ rivrctr2 ($815K), Withum Smith & Brown 25,306 SF @ rivrctr1 ($715K), JOT Mgmt 25K @ 200schlz ($774K). Total in-place annual base rent: ~$16.3M.
- **Parser:** `outputs/parse_rb.py` walks the pdftotext output, regex-detects MM/DD/YY lease-start/end pairs, locates the 6 money tokens (base_rent, op_cam, taxes, insurance, other, total) starting at `end_date + 1` (with optional notice-date token skip). Edge case: pdftotext occasionally smushes the tenant name and SF into one token when names overflow their column (e.g. "LLC2,579" or "Mgmt108,500") — handled by splitting trailing digit run back to SF. NVA Red Bank row was hopelessly mangled by pdftotext (name + SF + deposit + dates split across 2 lines with "L" leftover); hard-coded that single tenant in `outputs/insert_rb.py`.
- **Bulk insert script:** `outputs/insert_rb.py` creates the property record + inserts all rent_roll rows via direct REST API calls. Includes the `as_of_date=2026-04-30` for the period banner. Notes column carries the original Paramount tenant code (`code=rc1onef` etc.), notice date, deposit, and other_annual values for reference.
- **Photo not yet wired:** No entry in `FB_PHOTOS` yet — falls back to no header image. Add a photo to `assets/Property Photos/red-bank-river-center.png` and add `'Red Bank - River Center': 'assets/Property%20Photos/red-bank-river-center.png'` to `FB_PHOTOS` in index.html.
- **2026 Budget imported with monthly seasonality (2026-05-11):** Replaced the prior flat-monthly import using `2026 Budget.pdf` (Crown/FM underwriting model). 13 GL codes × 12 months = 156 rows. Real-world cash timing:
  - **Income:** 4010 Base Rent ($15.336M annual, varies $1.24M-$1.34M/mo) · 4110 CAM Recovery ($1.039M, varies $78-$142K, spike in Aug) · 4700 Other Income ($53.8K, $4,485/mo flat)
  - **OpEx:** 5497 R&M ($2.190M, $151-$211K/mo) · 5602 Utilities ($1.674M, $127-$157K/mo) · 5476 Engineers Payroll ($222K, $18.5K/mo flat) · 5654 Office Supplies / catch-all for Prof Fees + G&A ($158K) · 6501 Mgmt Fee 3.5% ($575K) · 6512 Real Estate Tax ($1.245M — quarterly Feb/May/Aug/Nov) · 6523 Property Insurance ($321K — annual premium August)
...
- **Git push from sandbox WORKS** (as of 2026-05-11). The `remote.origin.url` carries an embedded GitHub PAT (`https://ghp_TOKEN@github.com/...`), so the sandbox can `git push origin main` directly without keychain access. Workflow: commit locally + push from the sandbox in the same session. **Caveat:** the PAT is visible in `.git/config` — rotate it periodically and replace the token in the URL with `git remote set-url origin https://NEW_TOKEN@github.com/mzfirstmile/first-mile-claude.git`. If you ever drop the token from the URL, pushes from the sandbox will start failing again — they rely on credentials being baked into the remote URL since the sandbox can't reach the macOS keychain.
- **Stale .git lock files:** the sandbox's bind-mount can't unlink `.git/HEAD.lock` / `.git/index.lock` (`Operation not permitted` even though the user owns the file). Workaround: rename them aside before git operations — `python3 -c "import os; [os.rename(f'.git/{n}', f'.git/.stale.{n}.{os.getpid()}') for n in os.listdir('.git') if n.endswith('.lock') and not n.startswith('.stale')]"`. Renaming within `.git/` works; cross-filesystem moves return `EXDEV`. Always do this once before `git add` / `commit` if you've previously hit a failed git operation in the session.
...
- **Direct Dropbox access from sandbox (2026-05-18):** Morris whitelisted `api.dropboxapi.com` + `content.dropboxapi.com` in the sandbox proxy. Claude calls Dropbox via the `dropbox-proxy` edge function (`supabase/functions/dropbox-proxy/index.ts`) — never with a raw bearer token. The function:
  ...
  - **Actions supported:** `list_folder`, `list_folder_continue`, `get_metadata`, `search`, `search_continue`, `get_temporary_link`, `list_revisions`, `get_current_account`, `download` (returns base64), `download_text` (UTF-8), `upload` (writes a file — requires `args.content_base64` + `files.content.write` scope on the Dropbox app, added in proxy code 2026-06-03 but **needs deploy + scope grant + re-OAuth before it'll work**), `raw` (escape hatch — requires `args.path` for allowlist check).
  - **Upload helper script:** `scripts/upload_to_dropbox.py <local_path> <dropbox_path>` — handles base64 encoding + auth from config.js. Default mode = overwrite. To enable: (1) console.dropbox.com → First Mile Claude Assistant → Permissions → add `files.content.write` → Submit; (2) re-run OAuth authorize flow with `scope=` to get a new refresh token; (3) `supabase secrets set DROPBOX_REFRESH_TOKEN=<new>`; (4) `supabase functions deploy dropbox-proxy`.
...
- **"Loan Proceeds" category (added 2026-06-03):** Balance Sheet category for incoming debt (FM borrowing — cash in from a new loan or drawdown). Linkable to an `exec_liabilities` row; review/upload dropdowns offer "+ Add New Liability…" which prompts for lender / related deal / principal / maturity and creates a new liability row inline via `createLiabilityFromTxn()`. Computed under `category === 'loan_proceeds'` in `computePeriod()` with bucket `loanProceedsBuckets` + total `totalLoanProceeds`; rolls into cash flow with its natural sign (positive = cash in). Inverse of `Loan Out` — instead of creating an asset, it creates/feeds a liability.
