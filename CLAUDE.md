...
- **Financial data (balance sheet + budget comparison from PDF) NOT yet imported.** Paramount uses Yardi-ish GL codes (4500=Rent, 5200=Passthru Maintenance, 6210=R&M, etc.) that don't 1:1 map to First Mile's COA (4010 Base Rent, 4110 CAM Recovery, etc.). Skipped for v1 — Morris can map manually via SQL Console or wait for the next Paramount export with a translation layer.
