...
- **~~Investment distributions → auto-update distributed amount~~:** DONE 2026-03-30. When Investment Income transactions are linked to an investment, the `distributed` field on the balance sheet is computed dynamically by summing all linked Investment Income transactions. Updates in real time when linking/unlinking via `recomputeDistributed()`.
