...
async function handleCsvUpload(file) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Chase BAI2 has a running ledger balance in the "Balance/Value Date" column.
    // Read it so Cash on the balance sheet reflects post-upload balances.
    const balanceRaw = (cols[colIdx['Balance/Value Date']] || '').replace(/[",]/g, '').trim();
    const ledgerBalance = balanceRaw && !isNaN(parseFloat(balanceRaw)) ? parseFloat(balanceRaw) : null;
    ...
    rows.push({ date, description, amount, acctNum, acctName, creditDebit, bankRef, shortDesc, ledgerBalance });
  }
...
async function confirmImport() {
  ...
  const insertRows = uploadStagedTxns.map((txn, idx) => {
    ...
    const row = {
      ...
      description: txn.description,
      amount: txn.amount,
      ledger_balance: (txn.ledgerBalance != null && !isNaN(txn.ledgerBalance)) ? txn.ledgerBalance : null,
